...
- **自带Key（BYOK）**：访客可在「高级设置」填自己的得否 API Key，所有请求使用服务端固定的得否网关，
...
| 环境变量 | 默认值 | 说明 |
|---------|--------|------|
| `OPENAI_API_KEY` | 无 | 得否网关密钥（除非使用 Mock 模式，否则必填） |
| `AGENT_BASE_URL` | `https://api.wangdefou.studio/v1` | 固定的得否 OpenAI 兼容 API 地址；不接受其他网关 |
...
| `AGENT_CALL_DEADLINE` | `180` | 一次逻辑 LLM 调用的总时间预算（秒） |
...
AGENT_MOCK=1 .venv312/bin/python test_run_security.py     # 运行签名与会话安全
...
- **报错"未检测到API密钥"**：先 `export OPENAI_API_KEY=你的得否网关密钥`，或用 `AGENT_MOCK=1` 体验演示模式。
