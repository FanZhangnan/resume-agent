...
| `AGENT_REASONING_EFFORT` | 空（网关默认） | 推理强度（gpt-5.5实测支持）：`low`/`medium`/`high`/`xhigh`，另有`none`=关闭推理。Web UI可按次选择四档，CLI用`--reasoning`；换网关/模型后用`probe_reasoning.py`重新实测；不支持时自动忽略 |
