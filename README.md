...
| `AGENT_MODEL` | `gpt-5.5` | 模型名。`gpt-5.5` 是稳定基线；`gpt-5.6-terra` 和 `gpt-5.6-sol` 仅作实验引擎 |
| `AGENT_MOCK` | 关 | `=1` 开启离线演示模式 |
| `AGENT_REASONING_EFFORT` | 随模型 | GPT-5.5 支持 `none`/`low`/`medium`/`high`/`xhigh`（默认 `xhigh`）；Terra 支持 `xhigh`/`max`；Sol 支持 `high`/`xhigh`/`max`。非法组合会直接拒绝，不会静默降级 |
...
Web UI 会分别显示「稳定/实验」状态和「未定价/免费/付费」商业档位。GPT-5.5 的价格档暂未定义，
不会根据其稳定状态推断收费方式。选择 `max`
时必须每次页面会话明确确认。GPT-5.6 的名称不代表它在本项目中比 GPT-5.5 更快或更好。
